...
写一个对象来定义一个vue组件，该组件可以被h()渲染、在模版里直接使用、app.component()注册
...
即hyperscript（js、ts描述dom结构的写法），用于创建VNode的工具函数
...
el-form上的model属性指定表单使用的数据；rules属性设置表单中需要验证的表单项的验证规则

el-form-item上的prop属性，用于验证，指定为表单项对应model中的字段，如何不需要对该项验证可以不填

el-form-item下的el-input等组件使用v-model进行数据绑定，并不是el-form-item绑定了prop后就实现了数据绑定
...
只有当依赖数组中的值发生改变时，fn才是新的函数引用，否则使用上次缓存的函数
