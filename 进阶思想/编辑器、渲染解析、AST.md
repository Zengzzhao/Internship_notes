...
> 文档数节点通用属性：attrs节点属性，content子节点集合，marks节点上的mark，type节点类型，nodeSize为节点在 ProseMirror位置计数体系里占用的大小（在位置计数体系中，对于非叶子节点其所在区间是`开始token + 内容 + 结束token`，对于叶子节点其所在区间就是内容所在区间）
>
> 文档树中节点分为两类：
>
> TextNode（纯文本节点）作为叶子节点存在，没有子节点，属性：text为节点内的文本
>
> <img src="./编辑器、渲染解析、AST.assets/image-20260124114231496.png" alt="image-20260124114231496" style="zoom:40%;" />
>
> Node（类型结构节点）以纯文本节点TextNode作为子节点，比如'paragraph'、'heading'
>
> <img src="./编辑器、渲染解析、AST.assets/image-20260124114157131.png" alt="image-20260124114231496" style="zoom:40%;" />
...
> doc.descendants((node,pos,parent,index)=>booleam|void)：对文档树进行深度优先的先序遍历
...
>
> ```
> selection.from：选择起点位置（number），基于整个文档的绝对位置
> selection.to：选择终点位置（number），基于整个文档的绝对位置
> selection.anchor：锚点位置（number），基于整个文档的绝对位置
> selection.head：头部位置（number），基于整个文档的绝对位置
> selection.empty：是否为空选区（光标态）
> selection.$from / selection.$to：ResolvedPos解析位置对象（对象，将from解析成一个包含上下文信息的对象）
...
```js
// 传入一个ResolvedPos和某个mark类型，返回该mark的整体范围{from:number,to:number}，用于快速获取整个mark边界进行整体删除、替换等操作
getMarkRange(ResolvedPos,markType)
```
