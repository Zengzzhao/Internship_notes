...
| 文件路径 | 说明 |
|----------|------|
| `src/index.ts` | 子应用入口，生命周期函数，通信逻辑 |
| `src/router.ts` | 路由配置，动态加载，路由守卫 |
| `src/stores/projectStore.ts` | 项目状态管理 |
| `src/common/request/axios.ts` | HTTP 请求封装，错误处理通信 |
| `rsbuild.config.ts` | 构建配置，UMD 输出 |
| `src/modules/*/routes.ts` | 各模块路由定义 |
