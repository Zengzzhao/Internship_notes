...
```json
{
  ...
  "peerDependencies":{
    // 对等依赖:声明这个包运行时依赖的包。如果使用者工程中没有安装该包则包管理器会自动安装一个符合范围的该包安装；如果已经安装过该包且版本满足peer范围则不会再次安装，使用已经安装的版本；如果已经安装过该包且版本不满足peer范围则会报一个ERESOLVE的警告，不会报错error，使用的还是之前的包版本，不会再次安装一个满足peer范围的包的版本
  },
  "engines":{
    // 声明node、npm、pnpm版本要求
    "node": ">=22.14.0",
...
版本前缀的匹配策略：^只有主版本一致就匹配，~只有主版本、次版本一致就匹配、*不受版本号影响，全匹配
