...
tokenizer：将文本转换为token。tokenizer有一个预设的词汇库，记录了认识的所有token，用这个tokenizer去分割句子为一串token就是依赖的这个词汇库
...
经过残差链接&归一化（Add&Norm），将多头注意力每个头输出的改变量与原token的嵌入向量相加得到这个token最终的向量
