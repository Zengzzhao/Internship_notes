...
tokenizer分词器：将文本转换为token。tokenizer有一个预设的词汇库，记录了认识的所有token，使用tokenizer参考词汇库分割句子为一串token
...
将文本按照空格或者标点分割成token，缺点是无法处理未登录词（Out-of-Vocabulary，简称OOV），jieba分词就是基于这种分词方式
...
BPE（Byte-Pair Encoding）字节对编码：GPT使用

WordPiece：Google的预训练BERT
